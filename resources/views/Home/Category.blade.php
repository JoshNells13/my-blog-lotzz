...
@section('content')
    ...
    <div class="max-w-7xl mx-auto px-6 mb-16">
        ...
        <h1 class="text-4xl md:text-5xl font-bold text-white mb-4 italic">
            Category: <span class="text-neutral-500 not-italic">{{ $Category->name }}</span>
        </h1>
        <p class="text-neutral-500 max-w-2xl leading-relaxed">
            Menampilkan semua artikel dalam kategori <span class="text-white font-medium">{{ $Category->name }}</span>.
        </p>
    </div>
                ...
                <div class="relative w-full md:w-64 group">
                    <form action="{{ route('home.search') }}" method="GET">
                        <input type="text" name="query" value="{{ request('query') }}" placeholder="Cari..."
                            class="w-full pl-10 pr-4 py-2 bg-white/10 border border-white/20 rounded-xl text-sm focus:outline-none focus:border-white/30 transition-all" />
