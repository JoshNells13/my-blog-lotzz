    ...
    <div class="mb-12">
        <div class="flex gap-3 flex-wrap">
            ...
            <form action="{{ route('home.search') }}" method="GET">
                <input type="text" name="query" placeholder="Cari Blog Anda"
                    class="ml-auto px-4 py-2 bg-gray-800 text-gray-300 rounded text-sm font-medium transition-all hover:bg-gray-700"
                    />
            </form>
        </div>
    </div>
        ...
        @if ($Blogs->isNotEmpty())
            @foreach ($Blogs as $blog)
                <article
                    ...
                    <div class="p-6">
                        <span
                            class="text-xs font-medium text-gray-500 uppercase tracking-wider">{{ $blog->category->name }}</span>
                        ...
                        <div class="flex items-center justify-between pt-4 border-t border-gray-800">
                            ...
                            <a href="{{ route('home.blog.show', $blog->slug) }}"
                                class="text-gray-400 hover:text-white text-sm font-medium transition">
                                Baca →
                            </a>
                        </div>
                    </div>
            ...
            @endforeach
        @else
            <p class="text-gray-400">Tidak ada blog tersedia.</p>
        @endif
